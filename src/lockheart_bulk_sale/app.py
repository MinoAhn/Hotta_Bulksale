from __future__ import annotations

import queue
import threading
import time
from dataclasses import replace
from datetime import datetime
from pathlib import Path

import customtkinter as ctk
from PIL import Image

from .automation import GameAutomation, JobItem, StopRequested
from .config import AppSettings, load_settings, save_settings
from .materials import MATERIALS, PROJECT_ROOT, Material


ctk.set_appearance_mode("dark")
ctk.set_default_color_theme("dark-blue")

BG = "#070a12"
SURFACE = "#0d1422"
SURFACE_2 = "#121c2c"
SURFACE_3 = "#17243a"
BORDER = "#243348"
TEXT = "#f5f7fb"
MUTED = "#94a3b8"
TEAL = "#35d0ba"
BLUE = "#5ab8ff"
AMBER = "#f4bf5f"
RED = "#ef5b5b"


def asset_path(*parts: str) -> Path:
    return PROJECT_ROOT.joinpath(*parts)


class MaterialCard(ctk.CTkFrame):
    def __init__(self, master: ctk.CTkBaseClass, material: Material, settings: AppSettings) -> None:
        super().__init__(
            master,
            corner_radius=8,
            fg_color=SURFACE_2,
            border_width=1,
            border_color=BORDER,
        )
        self.material = material
        self.enabled = ctk.BooleanVar(value=settings.selected_enabled.get(material.slug, True))
        self.price = ctk.StringVar(value=str(settings.selected_prices.get(material.slug, material.default_price)))

        self.grid_columnconfigure(1, weight=1)

        icon = self._load_icon(material)
        icon_box = ctk.CTkFrame(self, width=58, height=58, corner_radius=8, fg_color="#1c2a42")
        icon_box.grid(row=0, column=0, rowspan=2, padx=(12, 10), pady=12)
        icon_box.grid_propagate(False)
        self.icon_label = ctk.CTkLabel(icon_box, image=icon, text="")
        self.icon_label.image = icon
        self.icon_label.place(relx=0.5, rely=0.5, anchor="center")

        self.name_label = ctk.CTkLabel(
            self,
            text=material.name,
            anchor="w",
            font=ctk.CTkFont(size=15, weight="bold"),
            text_color=TEXT,
        )
        self.name_label.grid(row=0, column=1, sticky="ew", padx=(0, 8), pady=(13, 2))

        meta = ctk.CTkLabel(
            self,
            text=f"参考价 {material.default_price}",
            anchor="w",
            font=ctk.CTkFont(size=12),
            text_color=MUTED,
        )
        meta.grid(row=1, column=1, sticky="ew", padx=(0, 8), pady=(0, 12))

        self.price_entry = ctk.CTkEntry(
            self,
            textvariable=self.price,
            width=86,
            height=34,
            justify="center",
            corner_radius=8,
            fg_color="#0a1020",
            border_color="#2e4568",
            font=ctk.CTkFont(size=15, weight="bold"),
        )
        self.price_entry.grid(row=0, column=2, rowspan=2, padx=(0, 10), pady=12)

        self.check = ctk.CTkSwitch(
            self,
            text="",
            variable=self.enabled,
            width=42,
            progress_color=TEAL,
            button_color="#e7eefb",
            button_hover_color="#ffffff",
        )
        self.check.grid(row=0, column=3, rowspan=2, padx=(0, 12), pady=12)

    def as_job(self) -> JobItem | None:
        if not self.enabled.get():
            return None
        try:
            price = int(self.price.get())
        except ValueError:
            raise ValueError(f"{self.material.name} 的价格不是整数。")
        if price <= 0:
            raise ValueError(f"{self.material.name} 的价格必须大于 0。")
        return JobItem(self.material.slug, price)

    def apply_to_settings(self, settings: AppSettings) -> None:
        settings.selected_enabled[self.material.slug] = self.enabled.get()
        try:
            settings.selected_prices[self.material.slug] = int(self.price.get())
        except ValueError:
            settings.selected_prices[self.material.slug] = self.material.default_price

    @staticmethod
    def _load_icon(material: Material) -> ctk.CTkImage:
        if material.icon_path.exists():
            image = Image.open(material.icon_path).convert("RGBA")
        else:
            image = Image.new("RGBA", (64, 64), "#223047")
        return ctk.CTkImage(light_image=image, dark_image=image, size=(46, 46))


class App(ctk.CTk):
    def __init__(self) -> None:
        super().__init__(fg_color=BG)
        self.title("Lockheart Bulksale")
        self.geometry("1180x760")
        self.minsize(1060, 680)
        self._set_window_icon()

        self.settings = load_settings()
        self.log_queue: queue.Queue[str] = queue.Queue()
        self.worker: threading.Thread | None = None
        self.automation: GameAutomation | None = None

        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(1, weight=1)

        self._build_header()
        self._build_workspace()
        self._build_log()
        self.after(100, self._drain_logs)

    def _set_window_icon(self) -> None:
        icon = asset_path("assets", "app_icon.ico")
        if icon.exists():
            try:
                self.iconbitmap(str(icon))
            except Exception:
                pass

    def _build_header(self) -> None:
        header = ctk.CTkFrame(self, corner_radius=0, fg_color=BG)
        header.grid(row=0, column=0, sticky="ew", padx=22, pady=(18, 10))
        header.grid_columnconfigure(1, weight=1)

        mark = ctk.CTkFrame(header, width=58, height=58, corner_radius=8, fg_color="#111b2b", border_width=1, border_color=BORDER)
        mark.grid(row=0, column=0, rowspan=2, padx=(0, 14), pady=2)
        mark.grid_propagate(False)
        mark_icon = self._load_header_icon()
        if mark_icon:
            label = ctk.CTkLabel(mark, image=mark_icon, text="")
            label.image = mark_icon
            label.place(relx=0.5, rely=0.5, anchor="center")
        else:
            ctk.CTkLabel(mark, text="LH", font=ctk.CTkFont(size=17, weight="bold"), text_color=TEAL).place(relx=0.5, rely=0.5, anchor="center")

        subtitle_label = ctk.CTkLabel(
            header,
            text="猎货批量上架工具",
            font=ctk.CTkFont(size=28, weight="bold"),
            text_color=TEXT,
            anchor="w",
        )
        subtitle_label.grid(row=0, column=1, sticky="ew")
        self.watermark_box = ctk.CTkTextbox(
            header,
            width=680,
            height=24,
            corner_radius=0,
            fg_color=BG,
            border_width=0,
            font=ctk.CTkFont(size=13),
            text_color=BG,
        )
        self.watermark_box.grid(row=1, column=1, sticky="ew", pady=(0, 0))
        self.watermark_box.insert(
            "1.0",
            "蔚色艾达-水銀灯 爱来自星海旅人 ♥ Copyright © Lockheart Bulk sale. All right reserved",
        )
        self.watermark_box.configure(state="disabled")

        self.status = ctk.CTkLabel(
            header,
            text="空闲",
            width=92,
            height=34,
            corner_radius=8,
            fg_color="#0e2430",
            text_color=TEAL,
            font=ctk.CTkFont(size=13, weight="bold"),
        )
        self.status.grid(row=0, column=2, rowspan=2, padx=(18, 0), sticky="e")

    def _load_header_icon(self) -> ctk.CTkImage | None:
        path = asset_path("assets", "app_icon_preview.png")
        if not path.exists():
            return None
        image = Image.open(path).convert("RGBA")
        return ctk.CTkImage(light_image=image, dark_image=image, size=(50, 50))

    def _build_workspace(self) -> None:
        workspace = ctk.CTkFrame(self, corner_radius=0, fg_color=BG)
        workspace.grid(row=1, column=0, sticky="nsew", padx=22)
        workspace.grid_columnconfigure(0, weight=5)
        workspace.grid_columnconfigure(1, weight=3)
        workspace.grid_rowconfigure(0, weight=1)

        self._build_material_panel(workspace)
        self._build_control_panel(workspace)

    def _build_material_panel(self, master: ctk.CTkFrame) -> None:
        shell = ctk.CTkFrame(master, corner_radius=8, fg_color=SURFACE, border_width=1, border_color="#1e2b40")
        shell.grid(row=0, column=0, sticky="nsew", padx=(0, 12))
        shell.grid_columnconfigure(0, weight=1)
        shell.grid_rowconfigure(0, weight=1)
        panel = ctk.CTkScrollableFrame(
            shell,
            corner_radius=8,
            fg_color=SURFACE,
            scrollbar_button_color="#2b405f",
            scrollbar_button_hover_color="#3f5f8c",
        )
        panel.grid(row=0, column=0, sticky="nsew", padx=0, pady=0)
        panel.grid_columnconfigure((0, 1), weight=1)

        title = ctk.CTkLabel(panel, text="材料队列", font=ctk.CTkFont(size=18, weight="bold"), text_color=TEXT)
        title.grid(row=0, column=0, padx=18, pady=(18, 4), sticky="w")
        hint = ctk.CTkLabel(panel, text="选择要处理的材料，并为每种材料设置上架价格", text_color=MUTED, font=ctk.CTkFont(size=12))
        hint.grid(row=1, column=0, columnspan=2, padx=18, pady=(0, 12), sticky="w")

        self.rows: list[MaterialCard] = []
        for index, material in enumerate(MATERIALS):
            row = MaterialCard(panel, material, self.settings)
            row.grid(row=index // 2 + 2, column=index % 2, padx=12, pady=8, sticky="ew")
            self.rows.append(row)

    def _button(self, master, text, command, color, hover, text_color=TEXT):
        return ctk.CTkButton(
            master,
            text=text,
            command=command,
            height=38,
            corner_radius=8,
            fg_color=color,
            hover_color=hover,
            text_color=text_color,
            font=ctk.CTkFont(size=14, weight="bold"),
        )

    def _build_control_panel(self, master: ctk.CTkFrame) -> None:
        shell = ctk.CTkFrame(master, corner_radius=8, fg_color=SURFACE, border_width=1, border_color="#1e2b40")
        shell.grid(row=0, column=1, sticky="nsew")
        shell.grid_columnconfigure(0, weight=1)
        shell.grid_rowconfigure(0, weight=1)
        side = ctk.CTkScrollableFrame(
            shell,
            corner_radius=8,
            fg_color=SURFACE,
            scrollbar_button_color="#2b405f",
            scrollbar_button_hover_color="#3f5f8c",
        )
        side.grid(row=0, column=0, sticky="nsew", padx=0, pady=0)
        side.grid_columnconfigure(0, weight=1)
        side.grid_rowconfigure(9, weight=1)

        ctk.CTkLabel(side, text="运行控制", font=ctk.CTkFont(size=18, weight="bold"), text_color=TEXT).grid(row=0, column=0, padx=18, pady=(18, 4), sticky="w")
        ctk.CTkLabel(side, text="扫描会自动最小化本程序，避免遮挡游戏画面", font=ctk.CTkFont(size=12), text_color=MUTED).grid(row=1, column=0, padx=18, pady=(0, 12), sticky="w")

        quick_actions = ctk.CTkFrame(side, corner_radius=8, fg_color="#0a1020", border_width=1, border_color="#1f314b")
        quick_actions.grid(row=2, column=0, padx=14, pady=(0, 12), sticky="ew")
        quick_actions.grid_columnconfigure((0, 1), weight=1)

        self.side_scan_btn = self._button(quick_actions, "扫描界面", self.scan_screen, BLUE, "#449ee0")
        self.side_scan_btn.grid(row=0, column=0, padx=8, pady=(10, 6), sticky="ew")
        self.side_start_btn = self._button(quick_actions, "开始上架", self.start_list, TEAL, "#28b59f")
        self.side_start_btn.grid(row=0, column=1, padx=8, pady=(10, 6), sticky="ew")
        self.side_remove_btn = self._button(quick_actions, "下架所选", self.start_remove, AMBER, "#dca84a", text_color="#101827")
        self.side_remove_btn.grid(row=1, column=0, padx=8, pady=(6, 10), sticky="ew")
        self.side_stop_btn = self._button(quick_actions, "停止", self.stop, RED, "#cf4848")
        self.side_stop_btn.grid(row=1, column=1, padx=8, pady=(6, 10), sticky="ew")

        self.title_keyword = ctk.StringVar(value=self.settings.window_title_keyword)
        self.threshold = ctk.StringVar(value=str(self.settings.match_threshold))
        self.interval = ctk.StringVar(value=str(self.settings.action_interval))
        self.dry_run = ctk.BooleanVar(value=self.settings.dry_run)

        self._field(side, "窗口标题关键字", self.title_keyword, 3)
        self._field(side, "识别阈值", self.threshold, 4)
        self._field(side, "操作间隔秒", self.interval, 5)

        dry_box = ctk.CTkFrame(side, corner_radius=8, fg_color=SURFACE_2, border_width=1, border_color=BORDER)
        dry_box.grid(row=6, column=0, padx=14, pady=8, sticky="ew")
        dry_box.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(dry_box, text="安全模式", font=ctk.CTkFont(size=14, weight="bold"), text_color=TEXT).grid(row=0, column=0, padx=12, pady=(10, 2), sticky="w")
        ctk.CTkLabel(dry_box, text="开启后只写日志，不向游戏发送点击；默认关闭", font=ctk.CTkFont(size=12), text_color=MUTED).grid(row=1, column=0, padx=12, pady=(0, 10), sticky="w")
        ctk.CTkSwitch(dry_box, text="", variable=self.dry_run, progress_color=TEAL).grid(row=0, column=1, rowspan=2, padx=12, pady=10)

        ctk.CTkButton(
            side,
            text="保存配置",
            command=self.save,
            height=38,
            corner_radius=8,
            fg_color="#27364e",
            hover_color="#334866",
            font=ctk.CTkFont(size=14, weight="bold"),
        ).grid(row=7, column=0, padx=14, pady=(8, 12), sticky="ew")

        ctk.CTkLabel(side, text="识别报告", font=ctk.CTkFont(size=15, weight="bold"), text_color=TEXT).grid(row=8, column=0, padx=18, pady=(4, 8), sticky="w")
        self.scan_result = ctk.CTkTextbox(
            side,
            height=250,
            wrap="word",
            corner_radius=8,
            fg_color="#090f1c",
            border_width=1,
            border_color="#1f314b",
            text_color="#d8e3f0",
        )
        self.scan_result.grid(row=9, column=0, padx=14, pady=(0, 14), sticky="nsew")
        self.scan_result.insert("end", "扫描结果会显示在这里。\n")
        self.scan_result.configure(state="disabled")

    def _build_log(self) -> None:
        bottom = ctk.CTkFrame(self, corner_radius=8, fg_color=SURFACE, border_width=1, border_color="#1e2b40")
        bottom.grid(row=2, column=0, sticky="nsew", padx=22, pady=(12, 18))
        bottom.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(bottom, text="运行日志", font=ctk.CTkFont(size=15, weight="bold"), text_color=TEXT).grid(row=0, column=0, padx=14, pady=(12, 4), sticky="w")
        self.log_box = ctk.CTkTextbox(
            bottom,
            height=132,
            wrap="word",
            corner_radius=8,
            fg_color="#070c16",
            border_width=1,
            border_color="#1f314b",
            text_color="#dce7f4",
        )
        self.log_box.grid(row=1, column=0, padx=14, pady=(0, 14), sticky="nsew")
        self.log("准备就绪。建议流程：进入游戏商店界面 -> 点击“扫描界面” -> 查看识别报告 -> 设置材料与价格 -> 执行上架或下架。")

    def _field(self, master: ctk.CTkFrame, label: str, variable: ctk.StringVar, row: int) -> None:
        box = ctk.CTkFrame(master, corner_radius=8, fg_color=SURFACE_2, border_width=1, border_color=BORDER)
        box.grid(row=row, column=0, padx=14, pady=7, sticky="ew")
        box.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(box, text=label, anchor="w", text_color=MUTED, font=ctk.CTkFont(size=12)).grid(row=0, column=0, padx=12, pady=(8, 2), sticky="ew")
        ctk.CTkEntry(
            box,
            textvariable=variable,
            height=34,
            corner_radius=8,
            fg_color="#090f1c",
            border_color="#2b405f",
        ).grid(row=1, column=0, padx=12, pady=(0, 10), sticky="ew")

    def collect_settings(self) -> AppSettings:
        settings = replace(self.settings)
        settings.window_title_keyword = self.title_keyword.get().strip() or "幻塔"
        settings.match_threshold = float(self.threshold.get())
        settings.action_interval = float(self.interval.get())
        settings.dry_run = self.dry_run.get()
        for row in self.rows:
            row.apply_to_settings(settings)
        self.settings = settings
        return settings

    def save(self) -> None:
        try:
            save_settings(self.collect_settings())
            self.log("设置已保存。")
        except Exception as exc:
            self.log(f"保存失败: {exc}")

    def start_list(self) -> None:
        try:
            settings = self.collect_settings()
            jobs = [job for row in self.rows if (job := row.as_job()) is not None]
            if not jobs:
                self.log("没有选中任何材料。")
                return
            save_settings(settings)
            self.log("价格配置已保存，本次价格下次打开会继续使用。")
            self._start_worker(lambda bot: bot.run_list(jobs), settings)
        except Exception as exc:
            self.log(f"启动失败: {exc}")

    def start_remove(self) -> None:
        try:
            settings = self.collect_settings()
            save_settings(settings)
            self.log("价格配置已保存。")
            slugs = [row.material.slug for row in self.rows if row.enabled.get()]
            if not slugs:
                self.log("没有选中任何材料。")
                return
            self._start_worker(lambda bot: bot.run_remove_selected(slugs), settings)
        except Exception as exc:
            self.log(f"启动失败: {exc}")

    def scan_screen(self) -> None:
        try:
            settings = self.collect_settings()
            save_settings(settings)
            self.log("扫描准备：即将最小化本程序，避免遮挡游戏窗口。")
            self.iconify()
            self._start_worker(self._scan_worker, settings)
        except Exception as exc:
            self.log(f"扫描失败: {exc}")

    def stop(self) -> None:
        if self.automation:
            self.automation.request_stop()
        self.log("已请求停止。")

    def _start_worker(self, task, settings: AppSettings) -> None:
        if self.worker and self.worker.is_alive():
            self.log("已有任务正在运行。")
            return
        self.status.configure(text="运行中", fg_color="#1b2b32", text_color=TEAL)
        self.automation = GameAutomation(settings, self._thread_log)
        self.worker = threading.Thread(target=self._run_task, args=(task, self.automation), daemon=True)
        self.worker.start()

    def _run_task(self, task, bot: GameAutomation) -> None:
        try:
            task(bot)
        except StopRequested as exc:
            self._thread_log(str(exc))
        except Exception as exc:
            self._thread_log(f"任务失败: {exc}")
        finally:
            self.log_queue.put("__STATUS_IDLE__")

    def _scan_worker(self, bot: GameAutomation) -> None:
        self._thread_log("扫描步骤 1/4：等待窗口最小化完成。")
        time.sleep(0.65)
        self._thread_log("扫描步骤 2/4：截取当前屏幕。")
        bag, listing = bot.scan()
        self._thread_log(f"扫描步骤 3/4：背包区域识别到 {len(bag)} 个匹配，上架区域识别到 {len(listing)} 个匹配。")
        lines = ["背包区域:"]
        lines.extend([f"  {m.name} {m.confidence:.2f} @ {m.center}" for m in bag] or ["  未识别到材料"])
        lines.append("")
        lines.append("上架区域:")
        lines.extend([f"  {m.name} {m.confidence:.2f} @ {m.center}" for m in listing] or ["  未识别到材料"])
        self.log_queue.put("__SCAN__" + "\n".join(lines))
        self._thread_log("扫描步骤 4/4：识别报告已生成，正在恢复程序窗口。")
        self.log_queue.put("__RESTORE__")

    def log(self, message: str) -> None:
        stamp = datetime.now().strftime("%H:%M:%S")
        self.log_box.insert("end", f"[{stamp}] {message}\n")
        self.log_box.see("end")

    def _thread_log(self, message: str) -> None:
        self.log_queue.put(message)

    def _drain_logs(self) -> None:
        while not self.log_queue.empty():
            message = self.log_queue.get()
            if message == "__STATUS_IDLE__":
                self.status.configure(text="空闲", fg_color="#0e2430", text_color=TEAL)
                continue
            if message == "__RESTORE__":
                self.deiconify()
                self.lift()
                self.focus_force()
                continue
            if message.startswith("__SCAN__"):
                self.scan_result.configure(state="normal")
                self.scan_result.delete("1.0", "end")
                self.scan_result.insert("end", message.removeprefix("__SCAN__"))
                self.scan_result.configure(state="disabled")
                self.log("扫描完成。请在右侧识别报告确认材料名称、置信度和坐标，再执行上架/下架。")
                continue
            self.log(message)
        self.after(100, self._drain_logs)


def main() -> None:
    app = App()
    app.mainloop()
